...
def read_apple(apple_link):
    match = re.search(apple_regex,apple_link)
...
def enter_music_link(song_link):
    if "youtube" in song_link:
        return {
            ...
            "spotify" : youtube_to_spotify(read_youtube(song_link))
        }
    
    elif "spotify" in song_link:
        return {
            "youtube_music" : spotify_to_youtube(read_spotify(song_link)),
            "spotify" : song_link
        }
    elif "apple" in song_link:
        return {
            "youtube_music" : spotify_to_youtube(read_apple(song_link)),
            "spotify" : youtube_to_spotify(read_apple(song_link))
        }
